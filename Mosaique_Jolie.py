...
def mosaique(chemin_im_source):
	new_size = 25							#taille des petites images
...
#Recoloration des images
def Filtre(im_source_HSV,cop):
	pix_source_HSV = im_source_HSV.load()
	pix_copy_HSV = cop.load()

	for x in range (im_source_HSV.size[0]):
		for y in range (im_source_HSV.size[1]):
			h1,s1,v1 = pix_source_HSV[x,y]
			h2,s2,v2 = pix_copy_HSV[x,y]
			pix_copy_HSV[x,y] = h2,(s1+s2)/2,(v1+v2)/2
